...
SELECT language FROM countrylanguages WHERE countrycode = (SELECT code FROM countries WHERE name = 'Holy See (Vatican City State)') AND isofficial = TRUE;
Italian


-- Clue #3: We have new news on the classes Carmen attended – our gumshoes tell us she's moved on to a different country, a country where people speak only the language she was learning. Find out which nearby country speaks nothing but that language.

SELECT name FROM countries JOIN countrylanguages ON countries.code = countrylanguages.countrycode WHERE countrylanguages.language = 'Italian' AND countrylanguages.percentage = 100;
San Marino
